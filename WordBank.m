...
-(instancetype) init
{
    ...
    if (self)
    {
        ...
        if([wordToDef count] == 0)
        {
            [self populateWithSATWords];
        }
...
-(void) storeData
{
    ...
    [defaults setObject:wordToDef forKey:@"words"];
...
-(void) restoreData
{
    ...
    NSDictionary *restoredWords = [defaults objectForKey:@"words"];
    
    [wordToDef addEntriesFromDictionary:restoredWords];
    
}

-(void) populateWithSATWords
{
